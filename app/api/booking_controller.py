...
from app.crud.booking import create_booking_in_db
...
@booking_router.post("")
def get_available_rooms(booking: BookingCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
  ...
  if not room_is_available(db, booking.room_number, valid_times["start_datetime"], valid_times["end_datetime"]):
    raise HTTPException(status_code=400, detail="Room is unavailable")
  if valid_times["start_datetime"] == valid_times["end_datetime"]:
    raise HTTPException(status_code=400, detail="Booking start and end times cannot be the same")
  # make the booking
  new_booking = Booking(
    user_id=user_id,
